...
async function seed() {
  ...
  await pool.query(`
    ...
    ('admin',      'admin@seatmaster.com',   $1, 'Admin User',           'admin'),
    ('employee1',  'employee@seatmaster.com',$1, 'Event Manager',        'employee'),
    ('moderator',  'moderator@seatmaster.com',$1,'Content Moderator',    'employee'),
    ('johndoe',    'john@example.com',       $1, 'John Doe',             'customer'),
    ('janedoe',    'jane@example.com',       $1, 'Jane Smith',           'customer'),
    ('musicfan',   'music@example.com',      $1, 'Music Lover',          'customer'),
    ('sportsfan',  'sports@example.com',     $1, 'Sports Enthusiast',    'customer')
  `, [passwordHash]);
  ...
  // 3. Events
  await pool.query(`
    INSERT INTO events (title, description, category_id, venue_name, venue_address, city, state, event_date, image_url, total_tickets, available_tickets, base_price, is_featured) VALUES
  ...
  await pool.query(`
    ...
    (5, 4, ARRAY[51],          1, 500.00,  'confirmed',  'PayPal',      'Awaiting event date'),
    (6, 7, ARRAY[101, 102, 103], 3, 269.97, 'processing', 'Credit Card', 'Payment being verified'),
    (7, 1, ARRAY[3],           1, 349.99,  'pending',    'Credit Card', 'Payment pending confirmation')
  `);
  console.log('Purchases inserted.');

  // Mark sold tickets and decrement available_tickets
  await pool.query(`UPDATE tickets SET is_sold = true WHERE id IN (1, 2, 51, 101, 102, 103, 3)`);
  await pool.query(`UPDATE events SET available_tickets = available_tickets - 3 WHERE id = 1`);
  await pool.query(`UPDATE events SET available_tickets = available_tickets - 1 WHERE id = 4`);
  await pool.query(`UPDATE events SET available_tickets = available_tickets - 3 WHERE id = 7`);
  ...
  `);
  console.log('Contact messages inserted.');
